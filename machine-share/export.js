...
var HOME = os.homedir()
var MY_HOME = process.env['MY_HOME'] || HOME
var TMP = os.tmpdir()
...
function processConfig () {
  ...
  util.recurseJson(config, function (parent, key, value) {
    if (typeof value === 'string') {
      if (util.startsWith(value, path.join(MY_HOME, DM_CERTS_DIR))) {
        var name = path.basename(value)
        var realPath = value.replace(MY_HOME, HOME)
        util.copy(realPath, path.join(tmp, 'certs', name))
        value = path.join(MY_HOME, DM_CERTS_DIR, machine, name)
      }
      value = value.replace(MY_HOME, '{{HOME}}')
